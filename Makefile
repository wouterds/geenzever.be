...
.build-app: node_modules
	docker run --rm -v $(PWD):/code -w /code --env=BASE_URL=$(BASE_URL) --env=API_BASE_URL=$(API_BASE_URL) --env=MODE=$(MODE) --env=SENTRY_DSN=$(SENTRY_DSN) node:10-slim npm run build
	touch .build-app
